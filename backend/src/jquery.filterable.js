(function( $ ) {
  $.fn.filterable = function(opts) {
    ...
    var addFilter = function(heading, index) {
      console.log(heading.text()+" "+index);
      if (heading.text()=="ผู้ให้บริการ" ||heading.text()=="ผู้ใช้บริการ"||heading.text()=="สถานะ"||heading.text()=="ประเภทรถ"
          ||heading.text()=="อีเมล"||heading.text()=="ชื่อ - นามสกุล"||heading.text()=="เบอร์โทรศัพท์"){

      var title = 'ค้นหาข้อมูล: ' + $.trim(heading.text());
      var icon = ' <i class="fa fa-filter filterable" data-name="' + index + '" title="' + title + '"></i>';
      }
      icon = heading.append(icon).find('i.fa-filter');
      popOver(icon);
      return icon;
    };
